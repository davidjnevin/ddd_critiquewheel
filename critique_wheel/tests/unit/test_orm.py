...
def test_create_and_retrieve_member(session, valid_member, valid_work, valid_critique):
    # Arrange
    ...
    session.commit()
    retrieved_member = session.query(Member).filter_by(id=new_member.id).one()
...
def test_create_and_retrieve_work(
    session, valid_critique1, valid_critique2, valid_member, valid_work
):
    # Arrange
    # Arrange work
    ...
    session.commit()
    retrieved_work = session.query(Work).filter_by(id=new_work.id).one()

    # Assert
    assert retrieved_work.id == new_work.id
    assert retrieved_work.title == new_work.title
    assert retrieved_work.content == new_work.content
...
def test_create_and_retrieve_critique(
    session, valid_critique, valid_work, valid_rating
):
    # Arrange
    ...
    session.commit()
    retrieved_critique = session.query(Critique).filter_by(id=valid_critique.id).one()
    ...
    assert retrieved_critique.critique_weaknesses == new_critique.critique_weaknesses
...
# @pytest.mark.skip(reason="Throwaway test file for testing ORM functionality")
def test_create_and_retreive_rating(session, valid_critique, valid_rating):
    # Arrange
    new_critique = valid_critique
    new_rating = valid_rating
    new_rating._critique_id = new_critique.id
    ...
    assert retrieved_rating.score == int(RatingScore(5))


# @pytest.mark.skip(reason="Throwaway test file for testing ORM functionality")
def test_create_and_retreive_credit(session, valid_credit):
    # Arrange
