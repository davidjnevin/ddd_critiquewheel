...
assert os.path.exists(
    CREDIT_RULES_FILE_PATH
), f"File not found at {CREDIT_RULES_FILE_PATH}"  # type: ignore
...
class CreditManager:
    ...
    def __init__(
        ...
        self.id = transaction_id or TransactionId()
    ...
    def validate_credit_rules(cls, credit_rules):
        ...
        if not all(key in credit_rules for key in ["submission", "critique"]):
            raise ValueError(
                "YAML file must contain 'submission' and 'critique' rules."
            )
        ...
        for rule_type in ["submission", "critique"]:
            ...
            for rule in credit_rules[rule_type]:
                if not all(key in rule for key in ["max_words", "credits"]):
                    raise ValueError(
                        f"Each rule in '{rule_type}' must contain 'max_words' and 'credits'."
                    )

                if not (
                    isinstance(rule["max_words"], int) or rule["max_words"] == "max"
                ):
                    raise ValueError(
                        f"'max_words' in '{rule_type}' must be an integer or 'max'."
                    )
    ...
    def create(
        ...
        if transaction_type == TransactionType.WORK_SUBMITTED and (
            work_id is None or critique_id is not None
        ):
            raise ValueError("Work submission must only have an associated work_id.")
        if transaction_type == TransactionType.CRITIQUE_GIVEN and (
            critique_id is None or work_id is None
        ):
            raise ValueError(
                "Critique submission must have an associated critique_id and work_id."
            )
        return cls(
            member_id=member_id,
            work_id=work_id,
